...
import { put, post } from '@/api/alarm'
      ...
      this.$refs.detail.$refs.form.validate((valid) => {
        if (valid) {
          if (type === 'add') {
            ...
            const data = { crimeTime: transformDate(crimeTime), ...rest, orgId: this.orgId }
            post(data).then(res => {
              if (res.status === 200 && res.message === 'success') {
                this.$message.success('新增警情成功')
